...
//función que crea un mensaje dentro de una ventana negra durante un tiempo, en la parte inferior de la pantalla del celular
    ...
    switch(splittedQR[0]){
          ...
          this.restService.postValTicketVehiculo(this.dataVehiculo).then(dataSP =>{
            ...
            if(dataSP['name'] === 'HttpErrorResponse'){
              ...
              console.log('ERROR');
                ...
                this.navCtrl.setRoot(AprobacionPage, {dataQR:splittedQR, tramo:this.tramo, switch:this.escaneo_continuo});
              }
            else{
              this.navCtrl.setRoot(RechazoPage, {dataQR:splittedQR, tramo:this.tramo, resultado:this.resultadoSQL.resultado, switch:this.escaneo_continuo});
            }
          });
        }
        break;
      ...
      case '17':{ //Pasajero Offline
        ...
        if(this.data.val_seed == compararValSeed.toString()){
          ...
          this.navCtrl.setRoot(RechazoPage, {dataQR:splittedQR, tramo: this.tramo, switch:this.escaneo_continuo});
        }
        break;
      }
  ...
  aprobarRechazarTicketPersona(splittedQR){
    if(this.tramo.cruce.id_cruce == this.data.id_cruce && this.resultadoSQL.resultado == 8){
      ...
      this.navCtrl.setRoot(RechazoPage, {resultado:this.resultadoSQL.resultado, tramo:this.tramo, dataQR:splittedQR, switch:this.escaneo_continuo});
    }
